...
exports.getSupportExtantion = async (req,res) => {
  await fdb.query(
    `SELECT CAST(ERMISSION AS VARCHAR(100) CHARACTER SET WIN1251), CAST(REQUIREMENT AS VARCHAR(100) CHARACTER SET WIN1251) FROM v_support_extantion WHERE METHOD_ID = ${req.params.id}`,
    (err, result) => {
      if (err) {
        return res.status(500).send({ msg: "Internal error", error: err });
      }
